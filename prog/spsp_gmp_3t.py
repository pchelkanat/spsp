            ...
            if (gmpy2.f_mod(pt, lambda_p) == gmpy2.f_mod(c, lambda_p)):
                ...
                if (num_for_check < search_range):
                    ...
                    if check_flag == True:
                        ...
                        print('p1--> %8s   p2-->%8s   p3-->%8s   base->: %s   spsp: %16s' % (
                        p_num[0], p_num[1], pt, a_bases, num_for_check))
    ...
    try:
        ...
        while (p_inner[0] < factor_range):
            while (p_inner[1] < factor_range):
                ...
                pool.apply_async(funcLoop, args=([int(p_inner[0]), p_inner[1]], search_range, a_bases,),
                                 callback=log_result)
            pool.close()
            pool.join()
